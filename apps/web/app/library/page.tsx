...
type Artifact = {
  ...
  processing_state?: CaptureStatus | ArtifactStatus | null;
  status?: CaptureStatus | ArtifactStatus | null;
  linked_project_count?: number | null;
  linked_note_count?: number | null;
  linked_task_count?: number | null;
  linked_card_count?: number | null;
  pending?: boolean;
  pendingLabel?: string;
};
...
function inferCaptureState(artifact: Artifact): CaptureStatus | ArtifactStatus {
  if (artifact.pending) {
    return "processing";
  }

  const explicit = artifact.processing_state || artifact.status;
  if (explicit) {
    return explicit;
  }
...
function artifactToEntry(artifact: Artifact): LibraryEntry {
  const sourceType = artifact.source_type || "capture";
  const captureType = artifact.capture_type || sourceType;
  return {
    id: artifact.id,
    title: artifact.title?.trim() || titleCase(sourceType),
    source: artifact.source?.trim() || artifact.source_url?.trim() || titleCase(sourceType),
    sourceUrl: artifact.source_url,
    captureType: titleCase(captureType),
    timestamp: artifact.updated_at || artifact.created_at,
    processingState: inferCaptureState(artifact),
    linkedProjects: Math.max(0, Number(artifact.linked_project_count || 0)),
    linkedNotes: Math.max(0, Number(artifact.linked_note_count || 0)),
    linkedTasks: Math.max(0, Number(artifact.linked_task_count || 0)),
    linkedCards: Math.max(0, Number(artifact.linked_card_count || 0)),
    kind: isInboxState(inferCaptureState(artifact)) ? "capture" : "artifact",
  };
}
...
function EntryActions({
  ...
  onAction: (entry: Pick<LibraryEntry, "id" | "title">, action: LibraryAction) => void;
}) {
  return (
    <div className={styles.actions} aria-label={`Actions for ${entry.title}`}>
      {ACTIONS.map((action) => (
        <button key={action} className={styles.actionButton} type="button" onClick={() => onAction(entry, action)}>
          {action}
        </button>
      ))}
    </div>
...
function EntryMetadata({ entry }: { entry: LibraryEntry }) {
  const totalLinks = linkedCount(entry.linkedProjects, entry.linkedNotes, entry.linkedTasks, entry.linkedCards);
  return (
...
function LibraryPageContent() {
  ...
  const loadLibrary = useCallback(async () => {
    try {
      const [artifactPayload, notePayload] = await Promise.all([
  ...
  const entries = useMemo(
    () => artifacts.map(artifactToEntry).sort((left, right) => right.timestamp.localeCompare(left.timestamp)),
    [artifacts],
  );
  ...
  const recentArtifacts = filteredEntries.filter((entry) => !isInboxState(entry.processingState)).slice(0, 6);
  const notesAndSavedItems = filteredNotes.slice(0, 6);
  const linkedProjects = entries.reduce((total, entry) => total + entry.linkedProjects, 0)
    + notes.reduce((total, note) => total + Math.max(0, Number(note.linked_project_count || 0)), 0);
  const breakdown = sourceBreakdown(inboxEntries);
  const sources = recentSources(entries);
  const nextInbox = inboxEntries[0];

  function handleAction(entry: Pick<LibraryEntry, "id" | "title">, action: LibraryAction) {
    setStatus(`${action} prepared for ${entry.title}`);
  }
    ...
    >
      <div className={styles.surface}>
        ...
        <section className={styles.stats} aria-label="Library stats">
          ...
          <div className={styles.stat}>
            <strong>{notesAndSavedItems.length}</strong>
            <span>Notes and saved items</span>
          </div>
          <div className={styles.stat}>
            <strong>{linkedProjects}</strong>
            <span>Linked to projects</span>
          </div>
        </section>
              ...
              <div className={styles.itemList}>
                {inboxEntries.length === 0 ? (
                  ...
                  <article key={entry.id} className={styles.row}>
                    ...
                    <EntryActions entry={entry} onAction={handleAction} />
                  </article>
                ))}
              </div>
            </section>
                  ...
                  return (
                    <article key={note.id} className={styles.noteCard}>
                      <div>
                        ...
                        <div className={styles.metadata}>
                          <span className={styles.typePill}>{titleCase(note.source_type || "note")}</span>
                          <span className={styles.statusPill} data-state="linked">Linked</span>
                          <span className={styles.metaText}>{note.source || "Starlog"}</span>
                          <span className={styles.metaText}>{formatDate(note.updated_at)}</span>
                          <span className={styles.metaText}>{Math.max(0, Number(note.linked_project_count || 0))} linked projects</span>
                        </div>
                      </div>
          ...
          <aside className={styles.rail} aria-label="Library context">
            ...
            <section className={styles.railCard}>
              ...
              <ul className={styles.suggestionList}>
                <li>
                  <strong>Summarize open articles</strong>
                  Turn the largest unread captures into short artifacts with source links.
                </li>
                <li>
                  <strong>Make cards from stable notes</strong>
                  Convert durable learning material into review items.
                </li>
                <li>
                  <strong>Link captures to projects</strong>
                  Attach relevant source material before creating tasks.
                </li>
              </ul>
            </section>
