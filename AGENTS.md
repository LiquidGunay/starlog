...
- 2026-03-14: Full `services/api/tests/test_api_flows.py` currently has expectation drift around execution-policy naming (`batch_local_bridge`/`api_fallback`/`local`) versus canonical returned targets (`desktop_bridge`/`api`), so those assertions fail even when unrelated worker-only changes are validated.
- 2026-03-14: CI mypy checks now enforce covariance safety on worker capability lists; worker/job service entrypoints needed `Sequence[str]` signatures instead of `list[str]` to accept schema literal lists without type errors.
- 2026-03-14: In this environment, pytest capture mode intermittently crashes with `FileNotFoundError` in `_pytest/capture.py`; running API tests with `-s` avoids the capture-layer failure and keeps test runs reliable.
