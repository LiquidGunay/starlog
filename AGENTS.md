...
- For subagent supervision, do not treat a missing final response after a short wait as a hang. Implementation workers should usually get a 10-15 minute `wait_agent` window, review workers an 8-10 minute window, then at most one status nudge followed by another long wait. Before closing a worker, inspect recent session activity; close only when there is no recent model/tool activity for roughly 15 minutes after the nudge, the worker is clearly blocked, or the user asks to stop. Prefer short worker prompts, avoid `fork_context` unless necessary, keep one writer per worktree/branch unless scopes are disjoint, and track worker id, task, start time, files changed, and next action.
