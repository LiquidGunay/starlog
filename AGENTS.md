...
- 2026-03-14: Assistant PWA offline voice-capture replay uses IndexedDB snapshot storage for queued media blobs with `persistBootstrap: false`; bootstrap localStorage is intentionally skipped so large binary payloads are not serialized into localStorage.
- 2026-03-14: The shared `playwright.web.config.ts` web-server command is hardcoded to `/home/ubuntu/starlog`; branch-local validation in Codex worktrees may require a temporary config or manually started web server rooted at the current worktree path.
