...
- 2026-03-14: Calendar event updates now accept optional `base_revision` and reject stale writes with structured `409 revision_conflict` payloads backed by persisted `/v1/conflicts` records for explicit resolution.
- 2026-03-14: Shelling `gh pr create --body "..."` with unescaped backticks triggers command substitution; use single-quoted heredoc/file input for PR bodies to avoid accidental command execution.
