...
- 2026-03-14: Mobile companion token persistence now uses OS secure storage (`expo-secure-store`) with one-time migration of legacy SQLite-stored tokens; app-state writes intentionally redact `token` to avoid plaintext local persistence.
