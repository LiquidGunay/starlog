...
- 2026-03-14: Agent execution-policy command parsing had drifted to legacy target names (`batch_local_bridge`/`server_local`/`api_fallback`) while policy storage normalizes to `mobile_bridge`/`desktop_bridge`/`api`, which caused `set ... policy` command flows to return `400` until canonical+legacy alias normalization was fixed.
