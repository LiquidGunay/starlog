...
- 2026-03-27: The conversation stack already persisted deterministic and AI-planned command turns, but it did not expose a persisted freeform chat-turn endpoint; WI-620 adds `/v1/conversations/primary/chat` so non-command turns share the same durable thread, session-state, card, and trace model.
...
- 2026-04-02: Remaining v1 closeout work should prioritize dynamic in-chat cards that update with thread state changes and SRS bootstrap decks derived from curated external question banks rather than another generic backlog pass.
- 2026-04-02: Queue refreshes must allocate fresh workitem IDs from the live registry before publishing new backlog entries; the human-readable queue briefly drifted and reused already-completed IDs, which is safe to fix only by updating the mirror and the registry together.
- 2026-04-02: SRS deck generation for the ML Interviews Part II bootstrap must prefer the public answer repository for real answers, key nested questions by their leaf text, and drop footer prose so the final deck count stays aligned with the book instead of drifting to a bogus extra card.
- 2026-04-02: The answer-repo parser must stay inside `QandA` blocks and skip container items with nested `InnerQandA`; otherwise the first emitted pair can swallow intro/footer prose and fall back to heuristic answers.
...
- 2026-04-06: On this host, `android_native_smoke.sh` can stall when Windows `adb.exe` is asked to install a WSL-side APK path directly; stage the APK into a Windows-visible path (or use the release smoke helper / direct `adb.exe install` against `C:\Temp\...`) before physical-device install validation.
- 2026-04-06: On this host, Windows `adb.exe install -r` can also remain silent during streamed installs; confirm the package actually updated by checking `dumpsys package com.starlog.app.dev | grep lastUpdateTime` before trusting the device proof.
