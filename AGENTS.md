...
- 2026-03-14: Desktop helper bearer-token persistence now uses OS keyring storage in Tauri runtime with one-time migration from legacy localStorage token values; browser-only fallback mode still keeps localStorage token behavior for non-Tauri runs/tests.
