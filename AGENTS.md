...
- 2026-03-14: Running multiple `pytest` processes in parallel from the same `services/api/.venv` hit a capture tmpfile race (`FileNotFoundError` inside `_pytest/capture.py`); use sequential runs or disable capture (`-s`) for reliable local verification in this environment.
