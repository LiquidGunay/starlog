...
- 2026-03-14: Synchronous `/v1/ai/run` intentionally skips `mobile_bridge`/`desktop_bridge` targets and falls through to `api`; tests that expected direct Codex-bridge execution in this sync path were outdated and needed to assert `provider_used == \"api\"`.
