...
- 2026-03-14: `services/api/tests/test_api_flows.py` had expectation drift around execution-policy naming (`batch_local_bridge`/`api_fallback`/`local`) versus canonical returned targets (`desktop_bridge`/`api`); assertions were updated to canonical values to keep API-flow regression coverage aligned.
