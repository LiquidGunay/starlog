...
Allowed v1 values are intentionally narrow: goal horizons are `today`, `week`, `month`, `quarter`, and `long_term`; goal and project statuses are `active`, `paused`, `completed`, and `archived`; commitment statuses are `open`, `done`, `dropped`, and `archived`.
