...
`PLAN.md` is the canonical forward-looking spec. For current Assistant UI targets and repeatable
PWA/mobile functional checks, use `docs/ASSISTANT_UI_REFERENCE.md`,
...
- Native mobile Assistant thread is now closer to desktop parity: the phone transcript renders `attachment`, `tool_call`, and `tool_result` parts inline instead of hiding them behind diagnostics, shared entity links can now route into `Library`, `Planner`, or `Review` from the thread, and the Assistant composer voice-state mapping now lives in a dedicated mobile helper instead of ad hoc conditionals in [apps/mobile/App.tsx](/home/ubuntu/starlog/apps/mobile/App.tsx).
- Mobile Assistant thread routing and card-action execution now also live outside the main shell in [apps/mobile/src/assistant-mobile-thread-actions.ts](/home/ubuntu/starlog/apps/mobile/src/assistant-mobile-thread-actions.ts), which keeps [apps/mobile/App.tsx](/home/ubuntu/starlog/apps/mobile/App.tsx) focused on orchestration instead of owning more assistant-specific thread navigation logic.
- The fresh-local Android validation harness is now reliable again on the connected phone: [scripts/android_fresh_local_srs_validation.sh](/home/ubuntu/starlog/scripts/android_fresh_local_srs_validation.sh) now derives the correct launch component from `APP_PACKAGE`, keeps the app foregrounded while polling auth/review UI, and avoids the earlier back-navigation bug during text entry, so the script completes the full login -> review queue -> reveal -> rate loop against a local API.
...
- Native mobile Assistant voice handling is now being centralized into a dedicated helper path: assistant mic state, clip-conflict copy, recorded-upload preflight, primary mic-button action selection, and the shared on-device STT lifecycle all now derive from [apps/mobile/src/assistant-mobile-voice.ts](/home/ubuntu/starlog/apps/mobile/src/assistant-mobile-voice.ts) instead of staying duplicated across multiple branches in [apps/mobile/App.tsx](/home/ubuntu/starlog/apps/mobile/App.tsx).
...
- Historical note: the semi-stable refresh on `2026-03-27` produced a fresh Android preview RC2 artifact (`0.1.0-preview.rc2`, code `103`, SHA-256 `0c9666daee9d4c6b99384de289a84a28b441b9d0a6d4f2271f387f251bdf8741`), a passing PWA release gate at `2026-03-27T18:17:09Z`, and a cross-surface proof bundle at the then-current tracked artifact path. This path is retained here only as historical chronology; current proof status belongs in `docs/CURRENT_STATE.md`, and dated incident/evidence history belongs in `docs/ENGINEERING_ISSUE_HISTORY.md`.
- The unified proof runner is now `./scripts/cross_surface_proof_bundle.sh`, which bundles hosted PWA, installed phone-app, and desktop-helper evidence into the ignored `.localdata/cross-surface-proof/latest/` bundle by default, with `STARLOG_CROSS_SURFACE_PROOF_BUNDLE_DIR` available for an explicit current-path override.
...
- `cd /home/ubuntu/starlog && ADB_SERIAL=9dd62e84 APP_VARIANT=preview APP_PACKAGE=com.starlog.app.preview APP_ACTIVITY=com.starlog.app.dev/.MainActivity SKIP_BUILD=1 EXISTING_APK_PATH=/home/ubuntu/starlog/apps/mobile/android/app/build/outputs/apk/release/app-release.apk bash ./scripts/android_fresh_local_srs_validation.sh`
...
1. Finish the remaining native mobile Assistant-shell cleanup: preserve per-tab state more intentionally, keep aligning mobile in-thread ambient/action states with the richer desktop thread treatment, and continue shrinking [apps/mobile/App.tsx](/home/ubuntu/starlog/apps/mobile/App.tsx) by extracting runtime/conversation helpers into focused modules.
2. Tighten the native mobile voice path beyond the new shared state derivation so the mic flow handles permission, listening, recording, stop/send, cancel, on-device STT, and recorded-upload fallback without state ambiguity.
