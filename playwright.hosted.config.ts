...
const artifactDir = requireSafeArtifactDir(
  process.env.STARLOG_PWA_HOSTED_SMOKE_ARTIFACT_DIR ?? path.join(repoRoot, ".localdata", "pwa-hosted-smoke", "latest"),
  "pwa-hosted-smoke",
  "STARLOG_PWA_HOSTED_SMOKE_ARTIFACT_DIR",
);
const testResultsDir = requireSafeOutputDir(
  process.env.STARLOG_PWA_HOSTED_TEST_RESULTS_DIR ?? path.join(artifactDir, "test-results"),
  "pwa-hosted-smoke",
  "STARLOG_PWA_HOSTED_TEST_RESULTS_DIR",
);
...
export default defineConfig({
  ...
  webServer: {
    command: `bash -lc "cd ${path.join(repoRoot, "apps/web")} && ./node_modules/.bin/next build && ./node_modules/.bin/next start --hostname 127.0.0.1 --port 3007"`,
